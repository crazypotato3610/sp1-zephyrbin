...
static constexpr std::array<uint16_t, 256> build_crc16_table() {
...
static constexpr auto crc16_table = build_crc16_table();

uint16_t EmmcDriver::crc16(const uint8_t* data, size_t len) {
    uint16_t crc = 0;
    for (size_t i = 0; i < len; ++i) {
...
bool EmmcDriver::spim3_transmit_bytes(const uint8_t* data, size_t len) {
    spim3_disable();

    // Clear stale events while disabled — must happen before re-enable
    NRF_SPIM3->EVENTS_END   = 0;
    NRF_SPIM3->EVENTS_ENDTX = 0;

    NRF_SPIM3->PSEL.MOSI = PIN_DAT0;
    NRF_SPIM3->PSEL.MISO = 0xFFFFFFFF;
    NRF_SPIM3->CONFIG     = 0;   // CPHA=0,CPOL=0: MOSI changes on falling CLK, stable by rising (eMMC sample) edge
    NRF_SPIM3->TXD.PTR    = reinterpret_cast<uint32_t>(data);
    NRF_SPIM3->TXD.MAXCNT = static_cast<uint32_t>(len);
    NRF_SPIM3->RXD.MAXCNT = 0;
    NRF_SPIM3->RXD.PTR    = 0;

    nrf_gpio_cfg(PIN_DAT0,
                 NRF_GPIO_PIN_DIR_OUTPUT,
                 NRF_GPIO_PIN_INPUT_DISCONNECT,
                 NRF_GPIO_PIN_NOPULL,
                 NRF_GPIO_PIN_H0H1,
                 NRF_GPIO_PIN_NOSENSE);
    nrf_gpio_pin_set(PIN_DAT0);

    spim3_enable();
    // Re-assert output config: spim3_enable() can call spim3_init_once()
    // which reconfigures DAT0 as input if spim3_configured_ was false.
    nrf_gpio_cfg(PIN_DAT0,
                 NRF_GPIO_PIN_DIR_OUTPUT,
                 NRF_GPIO_PIN_INPUT_DISCONNECT,
                 NRF_GPIO_PIN_NOPULL,
    ...
    while (NRF_SPIM3->EVENTS_ENDTX == 0) {
        if (--timeout == 0) {
            LOG_ERR("SPIM3 transmit timeout");
            spim3_disable();
            NRF_SPIM3->PSEL.MOSI = 0xFFFFFFFF;
            NRF_SPIM3->PSEL.MISO = PIN_DAT0;
            nrf_gpio_cfg_input(PIN_DAT0, NRF_GPIO_PIN_PULLUP);
    ...
    NRF_SPIM3->PSEL.MOSI = 0xFFFFFFFF;
    NRF_SPIM3->PSEL.MISO = PIN_DAT0;
    NRF_SPIM3->CONFIG     = 2;   // Restore CPHA=1,CPOL=0 for read path
    nrf_gpio_cfg_input(PIN_DAT0, NRF_GPIO_PIN_PULLUP);
    return true;
}
...
bool EmmcDriver::read_write_response() {
    volatile uint32_t* outset  = &NRF_P0->OUTSET;
    volatile uint32_t* outclr  = &NRF_P0->OUTCLR;
    const volatile uint32_t* in_reg = &NRF_P0->IN;

    bool found_start = false;
    for (int i = 0; i < 20 && !found_start; ++i) {
        *outset = CLK_MASK; clk_delay_fast();
        uint32_t bit = (*in_reg >> PIN_DAT0) & 1U;
        *outclr = CLK_MASK; clk_delay_fast();
        if (bit == 0U) found_start = true;
    }
    if (!found_start) {
        LOG_ERR("eMMC: no write response token");
        last_write_error = 3;
        return false;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        *outset = CLK_MASK; clk_delay_fast();
        uint32_t bit = (*in_reg >> PIN_DAT0) & 1U;
        *outclr = CLK_MASK; clk_delay_fast();
        status = static_cast<uint8_t>((status << 1) | bit);
    }
    // Clock the end bit
    *outset = CLK_MASK; clk_delay_fast();
    *outclr = CLK_MASK; clk_delay_fast();
...
bool EmmcDriver::wait_write_busy() {
    volatile uint32_t* outset  = &NRF_P0->OUTSET;
    volatile uint32_t* outclr  = &NRF_P0->OUTCLR;
    const volatile uint32_t* in_reg = &NRF_P0->IN;
    ...
    for (int i = 0; i < 30000; ++i) {
        ...
        *outclr = CLK_MASK; clk_delay_fast();
        if (dat0 != 0U) return true;
    }

    for (int i = 0; i < 500; ++i) {
        k_sleep(K_MSEC(1));
        // Provide clocking during busy — JESD84-B50 §6.4.5 requires it
        for (int j = 0; j < 16; ++j) {
            *outset = CLK_MASK; clk_delay_fast();
            *outclr = CLK_MASK; clk_delay_fast();
        }
    ...
    LOG_ERR("eMMC: write busy timeout");
    return false;
}

bool EmmcDriver::send_data_block(const uint8_t* data, uint8_t start_token) {
    alignas(4) uint8_t pkt[516];
    pkt[0] = 0xFF;
    pkt[1] = start_token;
    memcpy(&pkt[2], data, 512);
    uint16_t crc = crc16(data, 512);
    pkt[514] = static_cast<uint8_t>(crc >> 8);
    pkt[515] = static_cast<uint8_t>(crc & 0xFF);

    if (!spim3_transmit_bytes(pkt, 516)) { last_write_error = 2; return false; }
    if (!read_write_response()) { last_write_error = (last_write_error == 0) ? 3 : last_write_error; return false; }
    if (!wait_write_busy()) { last_write_error = 5; return false; }
    CLOCK_PULSES_FAST(8);
    return true;
}
...
bool EmmcDriver::cmd24_write_single(uint32_t block_addr, const uint8_t* data) {
    last_write_error = 0;
    if (!initialized_ || data == nullptr) return false;
    uint8_t resp[6];
    if (!send_command(24, block_addr, resp, 48)) { last_write_error = 1; return false; }
    const uint32_t r1 = r1_status(resp);
    if (r1 & 0xFDF98008U) {
        LOG_ERR("eMMC CMD24 R1 error: 0x%08x", r1);
        last_write_error = 1;
        return false;
    }
