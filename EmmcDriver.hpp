...
public:
    ...
    // Set by cmd24_write_single on failure; 0=none 1=R1err 2=tx 3=notoken 4=badstatus 5=busy
    int last_write_error = 0;
...
private:
    ...
    // CRC16-CCITT for eMMC write data blocks (polynomial x^16 + x^12 + x^5 + 1)
    ...
    bool spim3_transmit_bytes(const uint8_t* data, size_t len);
    void spim3_teardown();
    ...
    // Write data block helpers (CMD24/CMD25 use these)
